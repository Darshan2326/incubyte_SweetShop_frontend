...
//     const [allSweets, setAllSweets] = useState([]); // Store all sweets for search functionality
...
//                 setSweets(data);
//                 setAllSweets(data); // Store all sweets for search
//                 setLoading(false);
//             } catch (err) {
...
//         setSweets(allSweets); // Reset to all sweets
//     };
...
//             // Ensure the added sweet has all required properties with proper defaults
...
//             setAllSweets(prev => [...prev, formattedSweet]); // Also add to allSweets
...
//             // Validate the updatedSweet object before updating state
...
//         try {
//             const token = localStorage.getItem("token");
//             // Using the correct delete endpoint URL as specified
...
//             // Even if there's a CORS error, if the operation was successful, remove from UI
//             if (response.ok || response.status === 200 || response.status === 204) {
//                 setSweets(prev => prev.filter(sweet => sweet._id !== id));
//                 setAllSweets(prev => prev.filter(sweet => sweet._id !== id)); // Also remove from allSweets
//                 return;
//             }
...
//         } catch (err) {
//             // Even if there's an error, remove from UI since refresh shows it's deleted
//             setSweets(prev => prev.filter(sweet => sweet._id !== id));
//             setAllSweets(prev => prev.filter(sweet => sweet._id !== id)); // Also remove from allSweets
...
//         try {
//             const token = localStorage.getItem("token");
//             // Using the correct restock endpoint with query parameter
...
//             // Validate the updatedSweet object before updating state
//             if (updatedSweet && typeof updatedSweet === 'object') {
//                 // Find the existing sweet to preserve its data
...
//                     // Use the quantity from the API response, or calculate it if not provided
...
//                 <p style={styles.subtitle}>Manage your sweet shop inventory</p>
...
//                             <label style={styles.searchLabel}>Name</label>
//                             <input
//                                 type="text"
//                                 name="name"
...
//                         <div style={styles.searchGroup}>
//                             <label style={styles.searchLabel}>Min Price</label>
//                             <input
//                                 type="number"
//                                 name="low_price"
...
//                         <button type="button" onClick={handleClearSearch} style={styles.clearButton}>Clear</button>
//                     </div>
//                 </form>
//             </div>
...
//                     <h3 style={styles.formTitle}>{editingSweetId ? "Update Sweet" : "Add New Sweet"}</h3>
//                     <form onSubmit={editingSweetId ? handleUpdateSweet : handleAddSweet} style={styles.form}>
...
//             <div style={styles.sweetsContainer}>
//                 {sweets.length === 0 ? (
//                     <div style={styles.noSweets}>No sweets found.</div>
//                 ) : (
//                     sweets.map((sweet) => (
//                         <div key={sweet._id} style={styles.sweetCard}>
//                             <div style={styles.sweetHeader}>
...
//                             <p style={styles.sweetCategory}>{sweet.category}</p>
...
//                                     <p style={styles.historyTitle}>Last Action:</p>
